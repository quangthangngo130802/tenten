                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-10">
                                    <input type="{{ $field['type'] }}"
                                        ...
                                        @if (in_array($field['id'], ['company_phone', 'representative_phone', 'tax_id'])) oninput="formatNumberInput(this)"
                                        onpaste="handlePaste(event, this)" @endif />
                                    @error($field['id'])
...
@push('scripts')
    ...
    <script>
        function formatNumberInput(input) {
            input.value = input.value.replace(/\D/g, ""); // Chỉ giữ lại số, loại bỏ dấu chấm, khoảng trắng, ký tự đặc biệt
        }

        function handlePaste(event, input) {
            event.preventDefault(); // Ngăn chặn hành động paste mặc định
            let pastedData = (event.clipboardData || window.clipboardData).getData("text"); // Lấy nội dung dán vào
            input.value = pastedData.replace(/\D/g, ""); // Chỉ giữ lại số
        }
    </script>
    <script>
