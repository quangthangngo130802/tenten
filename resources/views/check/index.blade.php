...
@section('content')
    ...
    <h2 class="text-center my-4">KIỂM TRA TÊN MIỀN</h2>
    <div class="content">
...
@push('scripts')
    <script>
        ...
        document.getElementById("domainForm").addEventListener("submit", function(event) {
            ...
            if (domainInput === "") {
                showAlert("Vui lòng nhập tên miền cần kiểm tra");
                return;
            }

            fetch('/submit-check-domain', {
                    method: 'POST',
                    headers: {
                ...
                .then(result => {
                    ...
                    if (result.code == 1000) {
                        if (result.data.status == 0) {
                            showAlert(`Tên miền ${domainInput} đã tồn tại.`);
                        } else {
                            showAlert(`Tên miền ${domainInput} còn trống, bạn có thể đăng ký.`);
                        }
                    }else{
                        showAlert(`Tên miền ${domainInput} không khả dụng.`);
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    showAlert("Lỗi khi kiểm tra tên miền. Vui lòng thử lại.");
                });
        });
